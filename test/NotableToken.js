...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("NotableToken");
...
describe("NotableToken", () => {
  describe("Deployment", () => {
    it("name should be Notable", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("Notable");
    });

    it("symbol should be NBL", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("NBL");
    });
    ...
    it("total supply should be 100 000 000 tokens", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.totalSupply()).to.be.equal(tokens("100000000"));
    });

    it("deployer should have the total initial supply", async () => {
      const { pad, deployer } = await deployWithLGE();
      expect(await pad.balanceOf(deployer.address)).to.be.equal(tokens("100000000"));
    });
  });
